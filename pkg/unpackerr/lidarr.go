...
func (u *Unpackerr) importSplitFlacTracks(item *Extract, server *LidarrConfig) {
	...
	// Filter to split track files only, excluding the original unsplit audio file.
	// When NewFiles is populated (tracked by xtractr), use basename matching.
	// When NewFiles is empty (common after MoveFiles moves tracks back to the original folder),
	// filter to numbered track files (NN - Title.flac pattern) which are generated by CUE splitting.
	allOutputs := make([]*lidarr.ManualImportOutput, len(outputs))
	copy(allOutputs, outputs)

	outputs = filterManualImportToSplitTracks(allOutputs, item)

	if len(outputs) == 0 {
		u.Printf("[Lidarr] No split track files matched via NewFiles for %s; trying numbered track pattern", item.Path)
		outputs = filterManualImportToNumberedTracks(allOutputs)
	}
