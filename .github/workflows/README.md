...
- **Homebrew** — Developer ID-signed universal darwin tar.gz (archive id `darwin`; signed per slice by `init/macos/signbin.sh` in the build hook) → `golift/homebrew-mugs` `Casks/`. The cask pipe only accepts `UploadableArchive`/`UploadableBinary` artifacts, never DMG/App Bundle artifacts (v0.16.0 fatals: `no linux/macos archives found matching ids=[unpackerr-dmg]`). The notarized `Unpackerr.dmg` stays a manual-install release asset. Skip on `--nightly` (and that channel has no Darwin job).
