...
Append-only one line per persisted transition (`queued`, `extracting`, `extractfailed`, `extracted`, `extractednothing`, `imported`, `deleting`, `deleted`, `deletefailed`). Compact on load and when appends reach `2 × keep_history`. `histMu` covers records + file; HTTP reads, main loop appends. `GET /api/history` still returns only completed or failed rows. Cap, clear, and delete skip in-progress rows (`queued` / `extracting` / `extracted` / `deleting`) so a UI clear cannot wipe restart resume; delete of those IDs is **409**. The live hub upserts only durable rows and sends `delete` for an in-flight ID so the UI history table does not keep extracting items.
